...
def startCS(dict_css):
    ...
    for acc in dict_css:
        ...
        cs_w = subprocess.Popen([r'C:\Program Files (x86)\Steam\steam.exe','-login',str(login),str(password),'-applaunch','730','-low','-nohltv','-nosound','-novid','-window','-w','640','-h','480','+exec''autoexec.cfg', '+connect', '-x',str(counterX * (400 + px_between_cs)),'-y',str(counterY * 300)])
        ...
        pg.write(code)
        new_d[login] = [counterX * (400 + px_between_cs), counterY * 300, cs_w.pid]
        time.sleep(8)
        counterX += 1
